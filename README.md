<!-- Animated typing header -->

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=28&duration=3000&pause=800&color=36BCF7&center=true&vCenter=true&width=700&lines=Hi%2C+I'm+Kriszti%C3%A1n+Pletser;Full-Stack+Developer;React+%7C+Vue+%7C+TypeScript;Building+Modern+Web+Applications" alt="Typing SVG" />
</p>

# 👋 Hi, I'm Krisztian Pletser (@vxdudev)

🚀 **Full-Stack Developer** | 💼 One Hungary Plc. | 🇭🇺 Hungary, Budapest

---

## 👨‍💻 About Me

I'm a **23-year-old Full-Stack Developer** with **5+ years of hands-on development experience**, working professionally at **One Hungary Plc.**
I specialize in building modern, scalable, and user-focused web applications — from clean, responsive frontends to robust backends.

Alongside my full-time role, I work on **independent private development projects**, continuously improving my skills and exploring new technologies.

---

## 🛠️ Tech Stack

<!-- Animated tech badges -->

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,vue,nodejs,mongodb,git,npm&perline=5" />
</p>

### 🌐 Frontend (Expert)

* HTML5, CSS3 (Responsive / Modern layouts)
* JavaScript (ES6+)
* TypeScript
* React
* Vue.js

### ⚙️ Backend & Infrastructure (Medior)

* Node.js
* REST APIs
* MongoDB & NoSQL databases
* Authentication & Authorization
* npm / package management
* Git & version control

> 💡 *Comfortable with the complete modern web development ecosystem — from UI/UX implementation to backend logic and database design.*

---

## 💼 Professional Experience

### 🏢 One Hungary Plc.

**Full-Stack Developer**

* Developing and maintaining production-grade applications
* Working across frontend and backend systems
* Collaborating in professional, agile environments

---

## 🚧 Personal Projects

> 🔒 Most of my projects are **private independent developments**.

I focus on:

* Custom full-stack web applications
* Clean architecture & maintainable code
* Performance and scalability

*Public repositories and demos may appear here in the future.*

---

## 📊 GitHub Activity

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=vudeeh&show_icons=true&theme=transparent" height="160" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=vudeeh&theme=transparent" height="160" />
</p>

---

## 📈 Developer Mindset

> **"Code is not just about solving problems — it's about building solutions that last."**

* Passionate about clean code
* Strong problem-solving mindset
* Continuous learner
* Quality over quantity

---

<!-- Snake animation -->

<p align="center">
  <img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg" alt="Contribution Snake" />
</p>

## 📫 Contact

* 📧 Email: krisztianpl@icloud.com
* 🌍 Location: Hungary, Budapest

---

⭐ **Open to exciting challenges, collaboration, and growth opportunities.**

Thanks for stopping by!
